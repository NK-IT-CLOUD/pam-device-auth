...
// Wall-clock budget for the account-phase (--pam-acct) helper invocation.
// This mode does only a bounded busctl round-trip (~5s, see internal Go
// helper) plus a local /etc/passwd read — no LDAP/network calls of its own.
// Set a bit above the Go-side busctl bound so a clean per-user deny from the
// helper (exit 0/1) normally wins the race before this C-side timeout ever
// fires; the timeout is purely a backstop against a hung/wedged helper.
#define ACCT_TOTAL_TIMEOUT_S 7
...
// Extract client IP into caller-supplied buffer. Only PAM_RHOST is consulted:
// sshd sets it from the authenticated TCP connection, so it cannot be spoofed.
// The process environment (SSH_CONNECTION/SSH_CLIENT) is deliberately NOT used
// as a fallback — those variables don't exist in the PAM phase under normal
// sshd, and any value present (inherited unit env, AcceptEnv/
// PermitUserEnvironment misconfig) would be an attacker-influencable input to
// the OIDC IP-allowlist check. Without PAM_RHOST the IP stays "unknown" and
// the Go helper applies its fail-closed handling.
...
// if the deadline passes. Sets *timed_out to 1 iff the deadline fired and
// *wait_error to 1 iff the child's status could not be determined (waitpid
// error, e.g. ECHILD). Callers must treat *wait_error as fail-closed instead
// of interpreting the returned status: relying on WIFEXITED(-1) happening to
// be false is a Linux implementation accident, not a contract.
static int waitpid_until_deadline(pid_t pid, const struct timespec *deadline, int *timed_out, int *wait_error) {
    int status = -1;
    *timed_out = 0;
    *wait_error = 0;
...
PAM_EXTERN int pam_sm_authenticate(pam_handle_t *pamh, int flags, int argc, const char **argv) {
    ...
    // pam_get_user returns whatever the SSH client sent (up to ~255 bytes of
    // arbitrary data). Gate the length before the value enters the process
    // environment; 255 covers LOGIN_NAME_MAX on Linux with room to spare.
    if (strlen(username) > 255) {
        log_message(LOG_ERR, "Rejecting username longer than 255 bytes");
        return PAM_USER_UNKNOWN;
    }

    get_client_ip(pamh, client_ip, sizeof(client_ip));

    char safe_user[256], safe_ip[INET6_ADDRSTRLEN];
    ...
    // NOTE (concurrency): PAM_USER/PAM_RHOST are passed to the child via the
    // process environment. OpenSSH forks a dedicated process per connection for
    // auth, so this is safe in practice. Were PAM ever driven from multiple
    // threads in one process, this global env would race across logins; the
    // correct fix then is to pass identity over the to_child pipe instead of env.
    ...
    if (pid == 0) {
        ...
        // Close any leaked fds (syslog, etc.) — keep only 0,1,2. The auth pipes
        // are O_CLOEXEC and safe regardless; this also closes non-CLOEXEC fds up
        // to the actual descriptor limit instead of a hardcoded 1024.
        long maxfd = sysconf(_SC_OPEN_MAX);
        if (maxfd < 3 || maxfd > 65536) {
            maxfd = 65536;
        }
        ...
        // Dimensions derive from keep[] so adding a whitelist entry can never
        // desync the backing arrays (was a hardcoded 9 = latent stack overflow).
        char saved[sizeof(keep) / sizeof(keep[0])][2048];
        int saved_set[sizeof(keep) / sizeof(keep[0])] = {0};
    ...
    // Parent: read child stdout, relaying TEXT_INFO lines and FLUSH: prompts to
    // the SSH client via the PAM conversation. (The helper never prompts for a
    // password in directory mode, so there is no PROMPT:/secret-relay path.)
    ...
    for (;;) {
        ...
        if (strncmp(buffer, "FLUSH:", 6) == 0) {
            ...
            int conv_ret = conv->conv(total, ptrs, &resp, conv->appdata_ptr);
            ...
            // A failed conversation means the prompt never reached the user.
            // Acknowledging the child anyway ("phantom Enter") would start
            // the device flow invisibly; abort the attempt instead.
            if (conv_ret != PAM_SUCCESS) {
                log_message(LOG_ERR, "PAM conversation failed (%d) — aborting authentication", conv_ret);
                conv_failed = 1;
                break;
            }
            write_all(to_child[1], "\n", 1);
    ...
    // On a failed conversation the child is still mid-flow waiting on stdin/
    // polling the IdP; don't let it linger for the full deadline.
    if (conv_failed) {
        kill(pid, SIGKILL);
    }
    ...
    int waitpid_failed = 0;
    int status = waitpid_until_deadline(pid, &deadline, &waitpid_timed_out, &waitpid_failed);
    ...
    if (conv_failed) {
        result = PAM_CONV_ERR;
    } else if (waitpid_failed && !child_timed_out) {
        log_message(LOG_ERR, "Could not determine helper exit status");
...
PAM_EXTERN int pam_sm_acct_mgmt(pam_handle_t *pamh, int flags, int argc, const char **argv) {
    ...
    if (pam_get_user(pamh, &username, NULL) != PAM_SUCCESS) {
        log_message(LOG_ERR, "acct_mgmt: could not get username");
        return PAM_USER_UNKNOWN;
    }

    // Break-glass: root's account-phase decision must NEVER depend on the
    // helper binary, SSSD, or the IdP being reachable. Check this BEFORE any
    // fork/exec/network activity — a fork()+exec() that itself fails for an
    // unrelated reason (e.g. resource exhaustion) must not be able to touch
    // root at all.
    if (strcmp(username, "root") == 0) {
        return PAM_SUCCESS;
    }

    // Same length gate as pam_sm_authenticate: pam_get_user returns whatever
    // the SSH client sent before this value enters the process environment.
    if (strlen(username) > 255) {
        log_message(LOG_ERR, "acct_mgmt: rejecting username longer than 255 bytes");
        return PAM_USER_UNKNOWN;
    }
    ...
    sanitize_for_log(username, safe_user, sizeof(safe_user));
    sanitize_for_log(client_ip, safe_ip, sizeof(safe_ip));
    log_message(LOG_INFO, "Account check for user %s from IP %s", safe_user, safe_ip);
    ...
    pid_t pid = fork();
    if (pid < 0) {
        // Fork failure means we cannot even ask the helper. root is already
        // exempted above (break-glass); every other user fails closed rather
        // than silently bypassing the IP/role gate.
        log_message(LOG_ERR, "acct_mgmt: fork failed (%s) for user %s — denying login (fail-closed)", strerror(errno), safe_user);
        unsetenv("PAM_USER");
        unsetenv("PAM_RHOST");
    ...
    if (pid == 0) {
        // Child. This mode is non-interactive: the helper only returns an
        // exit code, there is no PAM conversation, so no pipes are wired up
        // — just close every fd beyond stdin/stdout/stderr.
        long maxfd = sysconf(_SC_OPEN_MAX);
        if (maxfd < 3 || maxfd > 65536) {
            maxfd = 65536;
        }
        ...
        // Sanitize environment before exec: identical whitelist/clearenv
        // pattern as pam_sm_authenticate's child (see the comment there for
        // the threat model — sshd may have inherited LD_PRELOAD,
        // LD_LIBRARY_PATH, IFS, or similar from its init unit or a
        // misconfigured AcceptEnv).
        static const char *const keep[] = {
            "PAM_USER",
            "PAM_RHOST",
            "PAM_DEVICE_AUTH_ISSUER",
        ...
        execl(AUTH_BIN, AUTH_BIN, "--pam-acct", (char *)NULL);
        _exit(127);
    }

    // Parent: block for the child's exit code, bounded by ACCT_TOTAL_TIMEOUT_S
    // so a hung/wedged helper cannot stall sshd indefinitely. A hung helper
    // is treated the same as any other helper-infrastructure failure: it
    // gets SIGKILLed and the login is denied (fail-closed) rather than
    // allowed. root is already exempted above, before any of this runs.
    struct timespec acct_deadline;
    deadline_from_now(&acct_deadline, ACCT_TOTAL_TIMEOUT_S);

    int acct_timed_out = 0;
    int acct_wait_error = 0;
    int status = waitpid_until_deadline(pid, &acct_deadline, &acct_timed_out, &acct_wait_error);

    if (acct_timed_out) {
        log_message(LOG_ERR, "acct_mgmt: helper timed out after %ds for user %s from IP %s — denying login (fail-closed)", ACCT_TOTAL_TIMEOUT_S, safe_user, safe_ip);
        unsetenv("PAM_USER");
        unsetenv("PAM_RHOST");
        return PAM_PERM_DENIED;
    }
    ...
    if (acct_wait_error) {
        log_message(LOG_ERR, "acct_mgmt: waitpid failed for user %s — denying login (fail-closed)", safe_user);
        unsetenv("PAM_USER");
        unsetenv("PAM_RHOST");
        return PAM_PERM_DENIED;
    }
    ...
    if (WIFEXITED(status)) {
        int exit_status = WEXITSTATUS(status);
        if (exit_status == 0) {
            log_message(LOG_INFO, "Account check passed for user %s from IP %s", safe_user, safe_ip);
            result = PAM_SUCCESS;
        } else if (exit_status == 1) {
            // A clean, definitive per-user deny from the helper (e.g. IP not
            // allowed, role revoked).
            log_message(LOG_WARNING, "Account check denied for user %s from IP %s", safe_user, safe_ip);
            result = PAM_PERM_DENIED;
        } else {
            // exit 127 (execl failed / AUTH_BIN missing) or any other
            // unexpected code is helper-infrastructure failure. root is
            // already exempted above (break-glass), so a missing or crashed
            // helper binary only ever denies directory users — it cannot
            // lock out the recovery path. Fail closed rather than silently
            // bypassing the IP/role gate.
            log_message(LOG_ERR, "acct_mgmt: helper exited %d (infra failure) for user %s from IP %s — denying login (fail-closed)", exit_status, safe_user, safe_ip);
            result = PAM_PERM_DENIED;
        }
    } else {
        // Killed by signal or otherwise abnormal exit — same fail-closed
        // reasoning as above.
        log_message(LOG_ERR, "acct_mgmt: helper terminated abnormally for user %s from IP %s — denying login (fail-closed)", safe_user, safe_ip);
        result = PAM_PERM_DENIED;
    }
