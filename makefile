...
deb: build-all
	...
	# -n: omit the source mtime+filename from the gzip header (reproducibility).
	gzip -9n $(DEB_DIR)/usr/share/man/man8/pam-device-auth.8
	chmod 755 $(DEB_DIR)/usr/local/bin/pam-device-auth
	chmod 755 $(DEB_DIR)/usr/lib/security/$(PAM_SO)
	@mkdir -p $(PACKAGES_DIR)
	# Reproducible .deb: pin every member mtime to the last commit time and force
	# root:root ownership so identical source yields byte-identical bytes/SHA256.
	export SOURCE_DATE_EPOCH=$$(git log -1 --format=%ct 2>/dev/null || echo 315532800); \
	find $(DEB_DIR) -exec touch --no-dereference -d @$$SOURCE_DATE_EPOCH {} + ; \
	dpkg-deb --root-owner-group --build $(DEB_DIR) $(PACKAGES_DIR)/pam-device-auth_$(VERSION)_amd64.deb
...
release: clean build-all deb
	...
	cp configs/config.json configs/config-auth0.json configs/config-okta.json \
	  ...
	  configs/pam-sshd-device-auth $(RELEASES_DIR)/pam-device-auth-$(VERSION)/configs/
