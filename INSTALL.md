...
- Ubuntu 24.04 or newer, or Debian 13 or newer, on amd64;
...
required because `--setup-ldap` supplies them. Set `gidNumber` on both access
and sudo groups; set each user's primary `gidNumber` to the access-group ID.

For an optional source-IP restriction, create the multi-valued `clients`
attribute, map it into the access token as a multi-valued String claim, and set
`"ip_claim": "clients"`. If the claim is absent or empty, no IP restriction is
...
Remove `ip_claim` if logins should not be restricted by source IP. Adjust
`allowed_algorithms` to the signing algorithm used by your provider.
...
drop-in and restarts sshd. It deliberately does not undo directory-mode host
configuration: `/etc/sssd/sssd.conf`, NSS `sss` entries, pam_mkhomedir,
`/etc/sudoers.d/pam-device-auth`, directory identities and home directories
remain. Review and remove those separately only when decommissioning the host.

## Build from source

Building creates the same Debian package used by the normal installation path:
...
[README configuration table](README.md#configuration), then follow steps 4–6
above.
