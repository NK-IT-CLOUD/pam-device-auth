...
An existing `/etc/ssh/sshd_config.d/10-pam-device-auth.conf` is left untouched by the upgrade, so a
host that installed pam-device-auth before the `AuthorizedKeysFile none` hardening (v0.5.4) keeps
its old drop-in until you act. Run `sudo pam-device-auth --setup-ldap` again (or update the drop-in
by hand) to pick up the hardening, and `sudo pam-device-auth --check` warns if it is still missing.
...
Any Keycloak-compatible OIDC provider that supports the Device Authorization
Grant (RFC 8628) works with the generic `config.json` layout.
...
The `ip_claim` feature lets the OIDC provider control which IPs each user can SSH from. The allowed IPs are stored as a user attribute in your identity provider and delivered as a signed JWT claim, so they cannot be tampered with on the server.
...
MaxAuthTries 3
LogLevel DEBUG1
# Non-root: SSH key (from the directory via SSSD) AND OIDC — comma = both
# factors required, ":pam" routes keyboard-interactive through PAM.
AuthenticationMethods publickey,keyboard-interactive:pam
# Local ~/.ssh/authorized_keys must be disabled for non-root, otherwise a
# same-named local account can bypass directory revocation.
AuthorizedKeysFile none
AuthorizedKeysCommand /usr/bin/sss_ssh_authorizedkeys
AuthorizedKeysCommandUser nobody

# Root: SSH key only (no OIDC required); local authorized_keys restored here
...
Verify the effective config resolves as expected:

```bash
sshd -T -C user=<someuser>,host=x,addr=127.0.0.1 | grep authorizedkeysfile   # expect: none
sshd -T -C user=root,host=x,addr=127.0.0.1 | grep authorizedkeysfile        # expect: .ssh/authorized_keys
```
...
- Verify `AuthenticationMethods publickey,keyboard-interactive:pam` is set (comma-separated; a space would make the factors alternatives instead of requiring both)
