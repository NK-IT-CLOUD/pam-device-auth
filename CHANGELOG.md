...
### Fixed
- **Session cache writes are safe against concurrent logins.** Each write
  uses a unique fsynced temp file, and load-modify-save cycles are serialized
  with a per-user lock; two simultaneous SSH sessions of the same user can
  no longer corrupt the cache or lose a rotated refresh token.
...
- **IP allowlist: exact entries now match across IPv4/IPv6 notation.** On
  dual-stack sshd a client arriving as `::ffff:10.0.1.2` matches the allowlist
  entry `10.0.1.2` (and vice versa) instead of being hard-denied.
...
- README: restored the APT-repository install instructions; clarified that **LDAPS is
  recommended, not required** (`ldap://`/StartTLS also work — the transport is whatever
  is in `ldap.uri`); removed the "Operating modes" section. man page and all example
