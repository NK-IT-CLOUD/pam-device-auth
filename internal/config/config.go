...
type Config struct {
	...
	// DirectoryMode switches the auth flow to "identity from the directory"
	// (LLDAP via SSSD/NSS). When true, pam-device-auth performs ONLY the OIDC
	// device-grant check (token + required_role + IP allowlist) as an extra SSH
	// login layer: no /etc/shadow access, no local password, no useradd/usermod,
	// no temp password. Identity, groups, home dir and the sudo password are
	// owned by SSSD/LLDAP. Use on hosts where users are provided by NSS.
