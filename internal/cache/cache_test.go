...
func TestWithUserLockSerializesWholeTransaction(t *testing.T) {
	setupTestDir(t)
	...
	go func() {
		errCh <- WithUserLock("nk", func() error {
			close(firstEntered)
			<-releaseFirst
			return nil
		})
	...
	select {
	case <-secondEntered:
		t.Fatal("second transaction entered while first held the per-user lock")
	...
	case <-time.After(time.Second):
		t.Fatal("second transaction did not enter after lock release")
	}
	for range 2 {
...
func TestInvalidUsername(t *testing.T) {
	...
	for _, name := range []string{"../etc/passwd", "root@domain", "", "a b", "name.ext", "a/b"} {
...
func TestMixedCaseUsername(t *testing.T) {
	setupTestDir(t)

	// LDAP / preferred_username values are case-preserving; a user whose
	// directory name contains uppercase must still get a working cache
	// (silent-refresh path), not just a one-shot device flow.
	for _, name := range []string{"NorbertK", "User1", "AB-cd_12"} {
		if err := Save(&CachedSession{Username: name, RefreshToken: "token"}); err != nil {
			t.Errorf("Save(%q) should succeed: %v", name, err)
		}
		session, err := Load(name)
		if err != nil {
			t.Errorf("Load(%q) should succeed: %v", name, err)
		}
		if session == nil || session.Username != name {
			t.Errorf("Load(%q) round-trip failed: %+v", name, session)
		}
	}
}
...
func TestAddIPIgnoresInvalid(t *testing.T) {
	s := &CachedSession{Username: "nk"}

	// "unknown" is the sentinel for an undetectable client IP; the cached
	// path never matches it, so storing it only wastes a FIFO slot.
	s.AddIP("unknown")
	s.AddIP("")
	if len(s.KnownIPs) != 0 {
		t.Errorf("KnownIPs = %v, want empty after invalid IPs", s.KnownIPs)
	}
...
func TestSaveLeavesNoTempFilesAndUniquePaths(t *testing.T) {
	CacheDir = t.TempDir()
	s := &CachedSession{Username: "alice", RefreshToken: "tok"}
	...
	for _, e := range entries {
		if e.Name() != "alice.json" {
			t.Errorf("unexpected leftover file %q", e.Name())
		}
	}
	info, err := os.Stat(filepath.Join(CacheDir, "alice.json"))
	if err != nil {
		t.Fatal(err)
	}
	if info.Mode().Perm() != 0600 {
...
func TestUpdateSerializesConcurrentWriters(t *testing.T) {
	...
	for i := 0; i < n; i++ {
		...
		go func(i int) {
			defer wg.Done()
			err := Update("bob", func(s *CachedSession) {
				s.RefreshToken = fmt.Sprintf("tok-%d", i)
				s.AddIP(fmt.Sprintf("10.0.0.%d", i%MaxIPs))
			})
			if err != nil {
	...
	got, err := Load("bob")
	if err != nil {
		t.Fatalf("cache corrupted after concurrent updates: %v", err)
	}
	if got == nil || !strings.HasPrefix(got.RefreshToken, "tok-") {
		t.Fatalf("unexpected final session: %+v", got)
	}
}

func TestUpdateCreatesSessionWhenMissing(t *testing.T) {
	CacheDir = t.TempDir()
	if err := Update("carol", func(s *CachedSession) { s.RefreshToken = "fresh" }); err != nil {
		t.Fatal(err)
	}
	got, err := Load("carol")
	if err != nil || got == nil || got.RefreshToken != "fresh" || got.Username != "carol" {
		t.Fatalf("got %+v, err %v", got, err)
	}
}

func TestUpdateRejectsInvalidUsername(t *testing.T) {
	CacheDir = t.TempDir()
	if err := Update("../evil", func(s *CachedSession) {}); err == nil {
