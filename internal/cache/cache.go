...
// for the same user via an exclusive flock on a per-user lock file. mutate
// receives the existing session, or a fresh one if no cache exists yet. The
// lock is advisory and Linux-only, which matches the PAM module's platform.
