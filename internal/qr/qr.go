...
// Supports QR versions 1-6, error correction level M, byte mode encoding.
// (Versions >= 7 are intentionally not produced: they require ISO/IEC 18004
// version-information blocks this encoder does not emit. The device-auth caller
// gates URLs to <= 84 bytes, which always fits version 5 or below.)
package qr
...
func selectVersion(dataLen int) (int, error) {
	// versionCapacity already holds the NET byte-mode data capacity (mode +
	// character-count overhead removed), so compare dataLen directly. Capped at
	// v6: versions >= 7 require the ISO/IEC 18004 version-information blocks this
	// encoder does not emit, which would yield an undecodable code.
	for v := 1; v <= 6; v++ {
		if dataLen <= versionCapacity[v] {
			return v, nil
		}
	}
	return 0, fmt.Errorf("qr: data too long (%d bytes), max %d for version 6", dataLen, versionCapacity[6])
}
