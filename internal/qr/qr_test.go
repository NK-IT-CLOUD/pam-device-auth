...
func TestEncode_LongURL(t *testing.T) {
	...
	// A ~190-char URL exceeds the v6 capacity (106 bytes). This encoder caps at
	// v6 — versions >= 7 need version-information blocks it does not emit — so it
	// must return an error rather than silently produce an undecodable code. The
	// device-auth caller gates URLs to <= 84 bytes (canRenderQR), so this is never
	// reached in practice.
	...
	if _, err := Encode(longURL); err == nil {
		t.Fatal("expected an error for a URL longer than version 6 capacity, got nil")
	}
}
