...
// The http://localhost dev exception must only apply when the issuer itself
// is localhost. A tampered discovery document for a production HTTPS issuer
// must not be able to smuggle JWKS retrieval past the host pin by pointing
// jwks_uri at a local listener.
func TestFetchRejectsLocalhostEndpointsForHTTPSIssuer(t *testing.T) {
	for _, host := range []string{"localhost", "127.0.0.1", "[::1]"} {
		t.Run(host, func(t *testing.T) {
			...
			_, err := Fetch(context.Background(), srv.Client(), srv.URL)
			if err == nil {
				t.Errorf("Fetch() should reject http://%s jwks_uri when the issuer is not localhost", host)
			}
		})
	}
}

func TestFetchAllowsIPv6LoopbackHTTP(t *testing.T) {
	endpoints := map[string]interface{}{
		"issuer":                        "http://[::1]:8080/realms/test",
		"token_endpoint":                "http://[::1]:8080/token",
		"device_authorization_endpoint": "http://[::1]:8080/device",
		"jwks_uri":                      "http://[::1]:8080/certs",
	}
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		json.NewEncoder(w).Encode(endpoints)
	}))
	...
	_, err := Fetch(context.Background(), srv.Client(), srv.URL)
	if err != nil {
		t.Fatalf("Fetch() should allow http://[::1] endpoints for a [::1] issuer: %v", err)
	}
}
