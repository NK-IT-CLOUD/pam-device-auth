...
// allowedAlgs, when non-empty, restricts the accepted `alg` header to that set;
// nil or empty accepts every supported algorithm (RS256/384/512, ES256/384/512).
func Validate(accessToken string, keys map[string]crypto.PublicKey, issuer, clientID, roleClaim, ipClaim string, allowedAlgs []string) (*TokenResult, error) {
	...
	// Reject tokens without a kid header. Paired with FetchJWKS dropping empty-
	// kid entries, this closes the empty-string collision path where a kid-less
	// JWT would otherwise pick up any key stored under "".
	if header.Kid == "" {
		return nil, fmt.Errorf("JWT header missing kid")
	}

	// Algorithm allowlist enforcement. Runs before any crypto so a poisoned
	// JWKS response cannot route us through an unexpected curve/hash. nil
	// or empty = accept any algorithm hashForAlgorithm supports.
	if len(allowedAlgs) > 0 {
		...
		if !permitted {
			return nil, fmt.Errorf("algorithm %q not in allowed_algorithms", header.Alg)
		}
	}
	...
	// Validate issued-at. iat is required — a token without iat bypasses
	// the forward-clock/replay check and every real Keycloak/Authentik/Okta
	// token carries it, so treat its absence as malformed. Accept a 60s
	// skew window (IdPs and PAM hosts can drift briefly even with NTP).
	iat, ok := claims["iat"].(float64)
	if !ok {
		return nil, fmt.Errorf("token missing iat claim")
	}
	const clockSkew = 60
	if time.Now().Unix()+clockSkew < int64(iat) {
		return nil, fmt.Errorf("token iat in future (%d, now=%d)", int64(iat), time.Now().Unix())
	}
