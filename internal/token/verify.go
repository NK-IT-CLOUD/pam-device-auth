...
func Validate(accessToken string, keys map[string]crypto.PublicKey, issuer, clientID, roleClaim, ipClaim string) (*TokenResult, error) {
	...
	// Validate issued-at. Accept a 60s skew window (IdPs and PAM hosts can
	// drift briefly even with NTP). Tokens with iat beyond that are either
	// forged or indicate a compromised clock on the IdP side — reject either.
