...
func TestValidate_Success(t *testing.T) {
	...
	result, err := Validate(jwt, keys, "https://sso.example.com/realms/test", "ssh-server", "", "", nil)
	if err != nil {
...
func TestValidate_ExpiredToken(t *testing.T) {
	...
	jwt := createRSATestJWT(t, privKey, kid, claims)
	_, err := Validate(jwt, keys, "https://sso.example.com/realms/test", "ssh-server", "", "", nil)
	if err == nil {
...
func TestValidate_InvalidJWTFormat(t *testing.T) {
	...
	_, err := Validate("not.a.jwt.token.at.all", keys, "x", "x", "", "", nil)
	if err == nil {
		t.Error("should fail for invalid JWT format")
	}

	_, err = Validate("only-one-part", keys, "x", "x", "", "", nil)
	if err == nil {
...
func TestValidate_AudienceFallback_String(t *testing.T) {
	...
	jwt := createRSATestJWT(t, privKey, kid, claims)
	if _, err := Validate(jwt, keys, "https://sso.example.com/realms/test", "ssh-server", "", "", nil); err != nil {
...
func TestValidate_ExtractsIPClaim(t *testing.T) {
	...
	jwt := createRSATestJWT(t, privKey, kid, claims)
	result, err := Validate(jwt, keys, "https://sso.example.com/realms/test", "ssh-server", "", "clients", nil)
	if err != nil {
		t.Fatalf("Validate() error: %v", err)
	}
...
func TestValidate_MissingIatRejected(t *testing.T) {
	privKey, _ := rsa.GenerateKey(rand.Reader, 2048)
	kid := "test-key-1"
	keys := map[string]crypto.PublicKey{kid: &privKey.PublicKey}
	...
	_, err := Validate(jwt, keys, "https://sso.example.com/realms/test", "ssh-server", "", "", nil)
	if err == nil {
		t.Fatal("should reject token with no iat claim")
	}
	if !strings.Contains(err.Error(), "iat") {
		t.Errorf("error should mention iat, got: %v", err)
	}
}
...
func TestValidate_AllowedAlgorithms_Accept(t *testing.T) {
	privKey, _ := rsa.GenerateKey(rand.Reader, 2048)
	kid := "test-key-1"
	keys := map[string]crypto.PublicKey{kid: &privKey.PublicKey}

	jwt := createRSATestJWT(t, privKey, kid, validClaims())
	_, err := Validate(jwt, keys, "https://sso.example.com/realms/test", "ssh-server", "", "", []string{"RS256"})
	if err != nil {
		t.Fatalf("RS256 token should be accepted when RS256 is allowed: %v", err)
	}
}

func TestValidate_AllowedAlgorithms_RejectUnlisted(t *testing.T) {
	privKey, _ := rsa.GenerateKey(rand.Reader, 2048)
	kid := "test-key-1"
	keys := map[string]crypto.PublicKey{kid: &privKey.PublicKey}

	jwt := createRSATestJWT(t, privKey, kid, validClaims())
	_, err := Validate(jwt, keys, "https://sso.example.com/realms/test", "ssh-server", "", "", []string{"ES256"})
	if err == nil {
		t.Fatal("RS256 token should be rejected when only ES256 is allowed")
	}
	if !strings.Contains(err.Error(), "allowed_algorithms") {
		t.Errorf("error should mention allowed_algorithms, got: %v", err)
	}
}

func TestValidate_AllowedAlgorithms_MultipleAllowed(t *testing.T) {
	privKey, _ := rsa.GenerateKey(rand.Reader, 2048)
	kid := "test-key-1"
	keys := map[string]crypto.PublicKey{kid: &privKey.PublicKey}

	jwt := createRSATestJWT(t, privKey, kid, validClaims())
	_, err := Validate(jwt, keys, "https://sso.example.com/realms/test", "ssh-server", "", "", []string{"RS256", "ES256"})
	if err != nil {
		t.Fatalf("RS256 token should be accepted when both listed: %v", err)
	}
}

func TestValidate_AllowedAlgorithms_EmptyTreatedAsNoRestriction(t *testing.T) {
	// An explicit empty slice must be treated the same as nil — no
	// enforcement — so a config-driven JSON unmarshal of `[]` does not
	// accidentally lock out all auth.
	privKey, _ := rsa.GenerateKey(rand.Reader, 2048)
	kid := "test-key-1"
	keys := map[string]crypto.PublicKey{kid: &privKey.PublicKey}

	jwt := createRSATestJWT(t, privKey, kid, validClaims())
	_, err := Validate(jwt, keys, "https://sso.example.com/realms/test", "ssh-server", "", "", []string{})
	if err != nil {
		t.Fatalf("empty allowlist should not block auth: %v", err)
	}
}
