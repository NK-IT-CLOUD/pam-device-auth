...
func TestSetup_RejectsGroupWithComma(t *testing.T) {
	// usermod -aG splits comma-separated group names on a single -aG argument.
	// A config entry like "sudo,root" would silently add the user to BOTH
	// groups — a privilege escalation if the config is compromised or wrong.
	// We must reject such values before invoking usermod.
	mock := &mockExecutor{}
	setupTestEnvironment(t, mock)

	_, _, err := Setup("testuser", []string{"sudo,root"}, nil, false, false, testLogger(t))
	if err == nil {
		t.Fatal("Setup() should reject group name containing comma")
	}
	for _, call := range mock.calls {
		if strings.Contains(call, "usermod") {
			t.Fatalf("usermod must not be invoked with invalid group: %v", mock.calls)
		}
	}
}

func TestSetup_RejectsGroupWithShellMeta(t *testing.T) {
	cases := []string{"sudo root", "sudo;root", "sudo$root", "sudo|root", "$(sudo)", "../etc"}
	for _, g := range cases {
		t.Run(g, func(t *testing.T) {
			mock := &mockExecutor{}
			setupTestEnvironment(t, mock)
			_, _, err := Setup("testuser", []string{g}, nil, false, false, testLogger(t))
			if err == nil {
				t.Fatalf("Setup() should reject group %q", g)
			}
		})
	}
}

func TestSetup_RejectsEmptyGroup(t *testing.T) {
	mock := &mockExecutor{}
	setupTestEnvironment(t, mock)
	_, _, err := Setup("testuser", []string{""}, nil, false, false, testLogger(t))
	if err == nil {
		t.Fatal("Setup() should reject empty group name")
	}
}

func TestSetup_AcceptsStandardGroups(t *testing.T) {
	// Baseline: common Linux group names must still be accepted.
	valid := []string{"sudo", "users", "wheel", "docker", "adm", "_systemd-network", "group-name", "group_name"}
	for _, g := range valid {
		if !validGroupName.MatchString(g) {
			t.Errorf("group %q should be valid", g)
		}
	}
}

func TestIsValidGroupName(t *testing.T) {
	invalid := []string{"", "Root", "123start", "has space", "way-too-long-group-name-that-exceeds-the-maximum", "special!char", "-start", "a,b", "a:b"}
	for _, g := range invalid {
		if validGroupName.MatchString(g) {
			t.Errorf("group %q should be invalid", g)
		}
	}
}
