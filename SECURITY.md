...
| Threat | Mitigation |
|--------|------------|
| Stolen SSH password | Mitigates only the `keyboard-interactive` path (OIDC re-checked on each fast-path login). A user authenticating via `publickey` alone does not enter the OIDC path — a stolen password grants no additional access for a key-holding user. |
...
### Scope of OIDC enforcement

OIDC (role, IP-allowlist, revocation) is enforced **only** when authentication uses the
`keyboard-interactive` method. A user authenticating with `publickey` alone bypasses OIDC
**by design** (this enables keyless self-onboarding for new users).

Consequences operators must know:
- Revoking a user's IdP role does **not** block SSH for a user who retains a valid SSH key
  and local account. `usermod --lock` only disables *password* auth, not key auth.
- The OIDC IP-allowlist binds only the OIDC path.
- **Full revocation** requires removing the user's `authorized_keys`/local account, or
  setting `AuthenticationMethods publickey,keyboard-interactive:pam` (comma = AND) and
  provisioning new users out-of-band.
