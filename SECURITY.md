...
- **OIDC token IP claim** (`ip_claim`): a second, independent IP mechanism,
  distinct from the directory `clients` allowlist above. This is an optional
  JWT claim containing allowed IPs or CIDRs per user, checked during the OIDC
  auth phase. The helper trusts the value only after token signature
