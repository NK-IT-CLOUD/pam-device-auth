...
- **Statically linked** — pure-Go helper, no CGO, no runtime dependencies
...
> **Using [LLDAP](https://github.com/lldap/lldap)?** Use **LLDAP ≥ 0.6.2** (first release with official SSSD support). It ships no POSIX schema, so add these as **custom attributes** (Admin UI → *Attributes*, or GraphQL): on users `uidNumber`, `gidNumber`, `sshPublicKey` (plus your `ip_claim` attribute — e.g. `clients` — if used); on the access **and** admin groups `gidNumber`. Built-in `uid` and `member` are used as-is — groups are `groupOfNames`/`member` (rfc2307bis), not `posixGroup`. You do **not** need `homeDirectory`/`loginShell`: the wizard sets `override_homedir`/`default_shell`.
