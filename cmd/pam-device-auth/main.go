...
func runPreflight() (fails, warns int) {
	...
	if cfg.LDAP != nil && cfg.LDAP.AccessGroup != "" {
		...
		if gerr == nil && len(members) > 0 {
			...
			if len(noKey) == 0 && len(badKey) == 0 {
				fmt.Printf("  [OK]   all %d access-group member(s) have a valid sshPublicKey (publickey factor)\n", len(members))
			} else {
				if len(noKey) > 0 {
					fmt.Printf("  [WARN] %d of %d member(s) have NO sshPublicKey — locked out of non-root login until set: %s\n",
						len(noKey), len(members), strings.Join(noKey, ", "))
					warns++
				}
				if len(badKey) > 0 {
					fmt.Printf("  [WARN] %d of %d member(s) have an UNPARSEABLE sshPublicKey — sshd will reject it: %s\n",
