...
func main() {
	...
	for _, arg := range os.Args[1:] {
		...
		case "--help":
			...
			fmt.Println("  --setup-ldap  Configure SSSD/LLDAP directory mode from config (NSS + sudo)")
	...
	// Directory mode: identity/groups/home/sudo-password come from the directory
	// (LLDAP via SSSD/NSS); pam-device-auth is only the OIDC SSH-login layer. No
	// /etc/shadow, no local password, no useradd. See directory.go.
...
// shouldRejectMissingLocalUser returns true when create_user=false and the
// user has no shadow entry at all (neither a usable hash nor a lock prefix),
// meaning they cannot be provisioned and must be rejected after OIDC success.
func shouldRejectMissingLocalUser(createUser bool, shadowHash string) bool {
	if createUser {
		return false
	}
	return !hasPasswordHash(shadowHash) && !isAccountLocked(shadowHash)
}
...
func tryCachedRefresh(log *logger.Logger, cfg *config.Config, httpClient *http.Client, endpoints *discovery.Endpoints, sshUser, clientIP string) (bool, bool) {
	...
	result, err := token.Validate(tokenResp.AccessToken, keys, endpoints.Issuer, cfg.ClientID, cfg.RoleClaim, cfg.IPClaim, cfg.AllowedAlgorithms)
	if err != nil {
...
func deviceAuthFlow(log *logger.Logger, cfg *config.Config, httpClient *http.Client, endpoints *discovery.Endpoints, sshUser, clientIP string, passwordVerified bool) {
	...
	// Enforce create_user=false: reject OIDC-authenticated users with no
	// local shadow entry. Runs before cache.Save to avoid persisting refresh
	// tokens for users who will never be valid locally.
	if shouldRejectMissingLocalUser(cfg.CreateUser, shadowHash) {
		log.Error("User %s has no local account and create_user=false — denying", sshUser)
		fmt.Println("------------------------------------")
		fmt.Println("Access denied: local account does not exist.")
		fmt.Println("Contact your administrator.")
		fmt.Println("------------------------------------")
		fmt.Println("FLUSH:Press Enter to disconnect... ")
		bufio.NewReaderSize(os.Stdin, 64).ReadString('\n')
		os.Exit(2) // PAM_PERM_DENIED — sshd won't retry
	}
