...
func runSetupLDAP() {
	...
	if !cfg.DirectoryMode {
		fail("directory_mode is not true in config — refusing to set up SSSD")
	}
	l := cfg.LDAP
...
func migrateLocalShadowing() {
	...
	for _, line := range strings.Split(string(data), "\n") {
		...
		dirUID, dirGID, home := p[2], p[3], f[5]
		fmt.Printf("Migrating shadowed local user %q: local uid %s -> directory uid %s\n", name, f[2], dirUID)
		exec.Command("pkill", "-KILL", "-u", name).Run()
		...
		if strings.HasPrefix(home, "/home/") {
			if out, err := exec.Command("chown", "-R", owner, home).CombinedOutput(); err != nil {
				fmt.Fprintf(os.Stderr, "  warn: chown %s failed: %s\n", home, strings.TrimSpace(string(out)))
			} else {
				fmt.Printf("  re-owned %s to %s\n", home, owner)
			}
		}
		for _, p := range []string{"/var/mail/" + name, "/var/spool/cron/crontabs/" + name} {
