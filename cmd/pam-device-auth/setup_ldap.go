...
// sshdDropinContent is the directory-mode sshd drop-in written by --setup-ldap.
// Non-root: publickey (served exclusively by sss_ssh_authorizedkeys — local
// authorized_keys files are disabled) AND keyboard-interactive (OIDC) — true
// 2FA whose first factor lives in the directory. Root: local key only
// (break-glass), restored via the Match block.
const sshdDropinContent = `# Managed by pam-device-auth --setup-ldap (directory mode)
...
func writeSshdDropin() error {
	...
	if err := atomicWriteFile(path, []byte(sshdDropinContent), 0644); err != nil {
		return err
	}
